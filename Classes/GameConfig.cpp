...
GameConfig *GameConfig::sharedInstance_ = nullptr;
...
GameConfig *GameConfig::sharedInstance() {
    if (sharedInstance_ == nullptr) {
        sharedInstance_ = new GameConfig();
    }
    ...
    if (designEntriesArray.IsArray()) {
        for (int i = designEntriesArray.Size() - 1; i >= 0; --i) {
            ...
            if (currentDeviceEntry.IsObject()) {
                float currentResolutionBaseDimension = (float) currentDeviceEntry[layoutBaseDimensionType.c_str()].GetDouble();
                ...
                if (frameBaseDimension > currentResolutionBaseDimension) {
                    ...
                    float requiredBaseDimension = (float) requiredEntry[layoutBaseDimensionType.c_str()].GetDouble();

                    float layoutWidth = (float) requiredEntry["designWidth"].GetDouble();
                    float layoutHeight = (float) requiredEntry["designHeight"].GetDouble();
